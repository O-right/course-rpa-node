...
- Assignment AI endpoint is currently healthy after chained-proxy and token refresh work, but it depends on the remote `glm` session cookie/access token and `mihomo` AI route staying valid.
...
- The latest live run left one assignment unsubmitted after selecting answers through question 26; question 27 requires manual review or a deliberate threshold change before any submit attempt.
- The enhanced-review path passed the previous Q27 stop point during a live run, but the run stopped later at Q45 because the AI endpoint connection was interrupted before retry support was added. A later retry attempt failed before login because Chaoxing login navigation timed out; login navigation retry support was added afterward.
- A live run after retry support submitted the biology assignment page after 50 processed questions because the old `ASSIGNMENT_MAX_QUESTIONS=50` cap was treated as normal completion; the cap-detection bug is fixed locally by preserving a `max_questions + 1` sentinel container. The same live run later stopped correctly on an unanswerable image/text fill-in question in another assignment.
...
- `glm/chat2api` checks now pass for the live authenticated path: SSH to `glm` works, services are running, `mihomo` AI route uses `ProxyPorts-US-ISP`, public `/v1/models` is 200, and authenticated `gpt-5.5` `/v1/chat/completions` returns 200.
- Latest assignment AI reliability checks passed: `python -m py_compile main.py assignment_tester.py`, local answer-normalization smoke, and a mocked multiple-choice review smoke where a first-pass `ABC` answer was corrected to `ABCD`.
- Latest image-question checks passed: local Playwright fixture captured one question-container PNG and built a multimodal payload, and `ask_ai_brain()` returned `["A"]` through `gpt-5.5` with a data URL image attached.
- Latest confidence-gate checks passed: local mock tests covered high-confidence continue, low-confidence halt, missing-confidence halt, and opt-out behavior; live `gpt-5.5` returned `{"answer":["B"],"confidence":1}` for a deterministic arithmetic smoke.
